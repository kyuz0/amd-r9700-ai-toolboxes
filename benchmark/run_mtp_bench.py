...
def discover_models(models_dir: Path) -> list[dict]:
    ...
    for gguf in sorted(models_dir.rglob("*.gguf")):
        ...
        # Calculate file size (handle multi-shard models)
        if "-00001-of-" in name:
            pattern = name.replace("-00001-of-", "-*-of-")
            size_bytes = sum(f.stat().st_size for f in gguf.parent.glob(pattern))
        else:
...
def start_server(toolbox: dict, gguf: str, models_dir: Path,
                 spec_flags: list[str], port: int, env_args: list[str] = None) -> bool:
    ...
    for tb_name, tb_config in toolboxes.items():
        for model in models:
            for mode_name, spec_flags in MODES.items():
                ...
                gpu_suffix = "__single"
                env_args = []
                if model["size_bytes"] > 24000000000:
                    gpu_suffix = "__dual"
                    if "rocm" in tb_name:
                        env_args = ["-e", "HIP_VISIBLE_DEVICES=0,1"]
                else:
                    if "rocm" in tb_name:
                        env_args = ["-e", "HIP_VISIBLE_DEVICES=0"]

                out_file = results_dir / f"{model['name']}__{tb_name}{gpu_suffix}__{mode_name}.json"

                print(f"\n{'─' * 80}")
                print(f"▶ [{run_count}/{total}] {model['name']}  |  {tb_name}  |  {mode_name}")
                print(f"  Size: {model['size_bytes'] / (1024**3):.1f} GB  |  Output: {out_file.name}")
